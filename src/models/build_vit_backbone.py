...
def build_vit_sup_models(
        model_type, crop_size, prompt_cfg=None, model_root=None, adapter_cfg=None, load_pretrain=False, vis=False
):
    ...
    if load_pretrain:  # 如果加载预训练模型
        root = os.path.join(model_root, MODEL_ZOO[model_type])
        model.load_from(np.load(os.path.join(model_root, MODEL_ZOO[model_type])))  # 加载预训练模型的权重
        print('Pre-training has been loaded')
