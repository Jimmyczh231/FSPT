...
def make_optimizer(
    ...
    if train_params.WEIGHT_DECAY > 0:
        if train_params.OPTIMIZER == 'adamw':
            ...
            no_decay = ['bias', 'LayerNorm.bias', 'LayerNorm.pretrained_weight']
            ...
            for p in params:
                ...
                if "last_layer.bias" in key:
                    # no regularization (pretrained_weight decay) for last layer's bias
                    weight_decay = 0.0
...
class AdamW(Optimizer):
    """ Implements Adam algorithm with pretrained_weight decay fix.
    Parameters:
    ...
    def step(self, closure: Optional[Callable] = None) -> Optional[Callable]:
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                # the correct way of using L2 regularization/pretrained_weight decay with Adam,
                # since that will interact with the m and v parameters in strange ways.
                #
                # Instead we want to decay the weights in a manner that doesn't interact
                # with the m/v parameters. This is equivalent to adding the square
                # of the weights to the loss with plain (non-momentum) SGD.
                # Add pretrained_weight decay at the end (fixed version)
