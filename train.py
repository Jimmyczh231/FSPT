...
def main(args):
    ...
    args.LUT_lr = [(10, 0.01), (20, 0.001), (30, 0.0001), (90, 0.00024)]
    ...
    for epoch in range(args.max_epoch):
        ...
        if epoch == 0 or epoch % 5 == 0:
            ...
            save_checkpoint({
                ...
                'acc': acc,
                'epoch': epoch,
            }, is_best, osp.join(args.save_dir, 'checkpoint_ep' + str(epoch + 1) + '.pth.tar'))  # save model
...
def train(args, epoch, model, criterion1, criterion2, optimizer, trainloader, learning_rate, use_gpu):
    ...
    for batch_idx, (images_train, labels_train, images_test, labels_test, pids_1, pids_2) in enumerate(
        ...
        trainer = Trainer()
        s1, s2, glo1, glo2, loss_con = trainer.trainer(model, images_train, labels_train_1hot, images_test, labels_test_1hot, labels_train, labels_test, args)
        ...
        loss = loss_global * args.weight_global + loss_xcos * args.weight_local + loss_con
...
def test(model, testloader, use_gpu):
    ...
    with torch.no_grad():
        for batch_idx, (images_train, labels_train, images_test, labels_test) in enumerate(tqdm(testloader)):
            ...
            trainer = Trainer()
            s3 = trainer.tester(model, images_train, labels_train_1hot, images_test, labels_test_1hot, labels_train, labels_test, args)
            ...
            gt = gt.view(q_batch_size, num_test_examples).numpy()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--height', type=int, default=96, help="height of an image (default: 96)")
    parser.add_argument('--width', type=int, default=96, help="width of an image (default: 96)")
    ...
    parser.add_argument('--train_epoch_size', type=int, default=1,
    ...
    parser.add_argument('--epoch_size', type=int, default=1, help='number of batches per epoch')

    parser.add_argument('--phase', default='val', type=str, help='use test or val dataset to early stop')
