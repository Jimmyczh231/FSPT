...
def default_argument_parser():
    ...
    parser.add_argument(
        "--config-file", default="configs/prompt/cub.yaml", metavar="FILE", help="path to config file")
    parser.add_argument(
...
def logging_train_setup(args, cfg) -> None:
    ...
    rank = get_rank()
    logger.info(
        f"Rank of current process: {rank}. World: {get_world_size()}")
    logger.info("environmental information:\n" + collect_env_info())

    logger.info("Command line parameters: " + str(args))
    ...
    logger.info("Use the following configuration for training:")
